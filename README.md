...
เมื่อ Agent ลงทะเบียนซ้ำด้วย enrollment token ที่ถูกต้อง API Center จะ revoke key เดิมของ Agent UID นั้นและออก Agent API Key ใหม่กลับไปให้ทันที เพื่อให้เครื่องที่ไฟล์ `agent.json` หาย ลงใหม่ หรือเคยเก็บ key ผิด endpoint สามารถกู้สถานะได้โดยไม่ต้อง rotate key ทีละเครื่องจาก Control
